...
ROWS = [
    ...
    ("9", "P2-01: API thiếu bảo mật và benchmark",
     "API used a default key file and no latency benchmark existed.",
     "src/api.py, run_api.bat, scripts/api_benchmark.py",
     "API_KEY env-only fail-closed auth (no default, 503 when unset); "
     "run_api.bat no longer uses --reload; api_benchmark.csv with "
     "p50/p95/max latency and fail-closed checks"),
    ("10", "P3-01: Báo cáo thiếu chuẩn hóa số liệu và hình",
     "Report numbers were partially hand-entered; figures not all referenced.",
     "generate_report.py, main.py",
     "Report regenerated automatically from outputs; dynamic counts throughout; "
     "all tables list data scope, n_samples and normalization"),
]
